...
async function initializeEmptyRepo(headers: Record<string, string>, owner: string, repo: string) {
  const putResponse = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/README.md`, {
    method: "PUT",
    headers,
    body: JSON.stringify({
      message: "Initialize repository",
      content: Buffer.from("# VS Code Extension\n\nGenerated with VS Code Extension Builder").toString("base64"),
    }),
  })
...
export async function POST(req: NextRequest) {
  try {
    const { token, owner, repo, files, commitMessage, isEmpty } = await req.json()
    ...
    if (!baseSha) {
      try {
        ...
        console.log("[v0] Could not initialize repo, trying without parent commit")
      }
    }

    try {
      const commitData = await pushWithGitDataApi(headers, owner, repo, files, commitMessage, baseSha)
      ...
      if (!refResponse.ok) {
        ...
        const altResponse = await fetch(`https://api.github.com/repos/${owner}/${repo}/git/refs/heads/${branch}`, {
          method: "PATCH",
          headers,
    ...
    return NextResponse.json({ error: "Failed to push files" }, { status: 500 })
  }
}
