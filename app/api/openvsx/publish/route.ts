...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!openVsxToken) {
      return NextResponse.json({ error: "Open VSX token is required" }, { status: 400 })
    }

    if (!publisher || !extensionName) {
      return NextResponse.json({ error: "Publisher and extension name are required" }, { status: 400 })
    }
    ...
    if (!ext) {
      return NextResponse.json({ error: "Failed to create extension folder" }, { status: 500 })
    }
    ...
    for (const [path, content] of Object.entries(files)) {
      if (path.endsWith(".png") && typeof content === "string" && content.startsWith("data:")) {
...
<PackageManifest Version="2.0.0" xmlns="http://schemas.microsoft.com/developer/vsx-schema/2011">
  ...
  <Assets>
    ...
    const namespaceCheckResponse = await fetch(`https://open-vsx.org/api/${publisher}`, {
      method: "GET",
      headers: {
    ...
    if (namespaceCheckResponse.status === 404) {
      ...
      const createNamespaceResponse = await fetch(`https://open-vsx.org/api/-/namespace/create?token=${openVsxToken}`, {
        method: "POST",
        headers: {
      ...
      if (!createNamespaceResponse.ok && createNamespaceResponse.status !== 409) {
        const errorText = await createNamespaceResponse.text().catch(() => "Unknown error")
    ...
    formData.append("file", vsixBlob, `${publisher}.${extensionName}-${version}.vsix`)

    const publishResponse = await fetch(`https://open-vsx.org/api/-/publish?token=${openVsxToken}`, {
      method: "POST",
      body: formData,
    })
    ...
    if (!publishResponse.ok) {
      ...
      return NextResponse.json(
        {
          error: (responseData.error as string) || (responseData.message as string) || "Failed to publish to Open VSX",
          suggestion: "Download the VSIX and publish manually using: npx ovsx publish <file> -p <token>",
          vsixBase64,
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Failed to publish to Open VSX",
        suggestion: "Try publishing manually using: npx ovsx publish -p <token>",
      },
      { status: 500 },
    )
  }
